...
router.put("/api/mock-tests/:testId/questions/:questionId", async (req, res) => {
  try {
    ...
    await mockTest.save();
    res.json({ message: "Question updated", updatedQuestion: question.toObject({ depopulate: true }) });
  } catch (error) {
    console.error("Error updating question:", error);
    res.status(500).json({ message: "Server error", error });
  }
...
router.delete("/mock-tests/:testId", async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({ message: "Mock test deleted successfully", deletedTest });
  } catch (error) {
    console.error("Error deleting mock test:", error);
    res.status(500).json({ message: "Server error", error });
  }
