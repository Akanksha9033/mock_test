...
const MockTests = () => {
  ...
  const handleToggleStatus = async (testId, currentStatus) => {
    ...
    try {
      const res = await fetch(`${REACT_APP_API_URL}/api/admin/mock-tests/${testId}/status`, {
        method: "PATCH",
          ...
          <div className="position-relative">
            ...
            {showFilterOptions && (
              ...
              >
                ...
                <button className="dropdown-item" onClick={() => setStatusFilter("active")}>
                  Active
                </button>
                <button className="dropdown-item" onClick={() => setStatusFilter("inactive")}>
                  Inactive
                </button>
              </div>
            )}
                ...
                <div className="card mb-3 shadow">
                  {test.wallpaper && (
                    <div
                      style={{
                        cursor:
                          test.status === "active" && user?.role?.toLowerCase() === "student"
                            ? "pointer"
                            : "not-allowed",
                      }}
                      onClick={() => {
