...
//       arr.map((item) => item.toLowerCase().trim()).sort().join(",");
...
//     } else {
//       isCorrect = Array.isArray(selectedOption)
//         ? normalizeArray(selectedOption) === normalizeArray(correctAnswer)
//         : selectedOption?.toLowerCase().trim() === correctAnswer[0]?.toLowerCase().trim();
//     }
...
const ExamContent = ({
  ...
  const handleFinishTest = async () => {
    ...
    try {
      const detailedAnswers = Object.entries(answers).map(([questionId, answerObj]) => {
        const originalQ = test.questions.find(q => (q._id || q.questionNumber) === questionId);
        if (!originalQ) console.warn(`⚠ Warning: Question with ID ${questionId} not found`);
        return {
          questionId,
          selectedAnswer: answerObj?.selectedOption || null,
          correctAnswer: answerObj?.correctAnswer || null,
